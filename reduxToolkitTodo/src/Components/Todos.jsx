...
function Todos() {
  ...
  return (
    <>
      <div className="text-white text-3xl">Todos</div>
      <ul className="list-none">
        {todos.map((todo) => (
          <li
            className=" w-96 mt-4 flex justify-between items-center bg-zinc-800 px-4 py-2 rounded"
            key={todo.id}
          >
            <div className="text-white">
              {editId === todo.id ? (
                <input 
                  ...
                  className="bg-gray-800 rounded border border-gray-700 text-gray-100 py-1 px-2 focus:ring-2 focus:ring-indigo-900 outline-none"
                />
              ) : (
                todo.text
              )}
